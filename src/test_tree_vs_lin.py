...
def test_special():
    ...
    assert(sorted(tree_runner(a10_fasta,a1_fastq))==sorted(lin_runner(a10_fasta,a1_fastq)))
    assert(sorted(tree_runner(a10_fasta,empty_fastq))==sorted(lin_runner(a10_fasta,empty_fastq)))
    assert(sorted(tree_runner(a1_fasta,a1_fastq))==sorted(lin_runner(a1_fasta,a1_fastq)))
    assert(sorted(tree_runner(empty_fasta,empty_fastq))==sorted(lin_runner(empty_fasta,empty_fastq)))
    assert(sorted(tree_runner(empty_fasta,a1_fastq))==sorted(lin_runner(empty_fasta,a1_fastq)))
...
def test_genome():
    ...
    fastq25 = fasta_func(genome_sequence_generator('src/sample_sequence.gz',
     25, 8, 'fastq', 'fastq25'))
    assert(sorted(tree_runner(gen,fastq0))==sorted(lin_runner(gen,fastq0)))
    assert(sorted(tree_runner(gen,fastq1))==sorted(lin_runner(gen,fastq1)))
    assert(sorted(tree_runner(gen,fastq10))==sorted(lin_runner(gen,fastq10)))
    assert(sorted(tree_runner(gen,fastq25))==sorted(lin_runner(gen,fastq25)))